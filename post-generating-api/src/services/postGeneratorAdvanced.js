...
class PostGenerator {
  ...
  async generatePostBuffer(sport, faculties) {
    ...
    const overlays = this.createTextOverlays(sport, faculties);
  ...
  createTextOverlays(sport, faculties) {
    ...
    const sportSvg = `
      <svg width="1400" height="1400">
        ...
        <text x="${this.config.sportPosition.x}" y="${this.config.sportPosition.y}" class="sport-text">
      ...
      if (index < this.config.facultyPositions.length) {
        ...
        const facultySvg = `
          <svg width="1400" height="1400">
            ...
            <text x="${pos.x}" y="${pos.y}" class="faculty-text">
  ...
  async processTemplate(overlays) {
    try {
      ...
      throw new Error(`Failed to process template: ${err.message}`);
    }
  }
